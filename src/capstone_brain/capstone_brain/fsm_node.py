...
class SoccerFSMNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('approach_curve_turn_hold_speed', 0.18)
        self.declare_parameter('approach_curve_turn_stuck_breakaway_speed', 1.90)
        self.declare_parameter('approach_curve_turn_stuck_hold_speed', 0.45)
        self.declare_parameter('approach_close_turn_breakaway_speed', 0.70)
        self.declare_parameter('approach_close_turn_hold_speed', 0.22)
        self.declare_parameter('approach_close_turn_stuck_breakaway_speed', 0.90)
        self.declare_parameter('approach_close_turn_stuck_hold_speed', 0.30)
        ...
        self.declare_parameter('ball_chase_center_threshold_px', 95.0)
        self.declare_parameter('ball_chase_center_exit_threshold_px', 120.0)
        self.declare_parameter('ball_close_center_threshold_px', 60.0)
        self.declare_parameter('ball_close_center_exit_threshold_px', 80.0)
        self.declare_parameter('ball_near_err_y_threshold_px', 120.0)
        self.declare_parameter('ball_near_center_threshold_px', 45.0)
        self.declare_parameter('ball_near_center_exit_threshold_px', 65.0)
        self.declare_parameter('ball_close_steer_band_px', 150.0)
        self.declare_parameter('ball_close_center_area', 4500.0)
        self.declare_parameter('approach_sign_flip_guard_px', 220.0)
        self.declare_parameter('approach_sign_flip_hold_sec', 0.18)
        self.declare_parameter('approach_close_sign_flip_guard_px', 90.0)
        self.declare_parameter('approach_close_sign_flip_hold_sec', 0.12)
        ...
        self.declare_parameter('visible_possession_close_min_area', 6000.0)
        self.declare_parameter('visible_possession_close_max_err_x', 60.0)
        self.declare_parameter('visible_possession_close_max_err_y', 55.0)
        self.declare_parameter('visible_possession_close_recent_center_sec', 0.20)
        self.declare_parameter('approach_plow_brake_min_area', 6000.0)
        self.declare_parameter('approach_plow_brake_max_err_x', 80.0)
        self.declare_parameter('approach_plow_brake_max_err_y', 55.0)
        ...
        self.approach_close_area_mode = False
        self.approach_near_ball_mode = False
    ...
    def control_loop(self):
            ...
            if visible_ball:
                ...
                if centered_enough:
                    ...
                    centered_plow_brake_ready = (
                        ...
                        and abs(error_x) <= float(self.get_parameter('approach_plow_brake_max_err_x').value)
                    )
                    if centered_plow_brake_ready:
                    ...
                    if near_ball_mode:
                        ...
                        turn_limit = float(self.get_parameter('ball_close_steer_turn_speed').value) if close_area_mode else float(self.get_parameter('ball_chase_max_turn_speed').value)
                    twist.angular.z = self.biased_turn(
                        error_x,
                    ...
                    if close_area_mode:
                        ...
                        heading_scale = max(
                            ...
                            1.0 - (abs(error_x) / max(float(self.get_parameter('ball_arc_heading_slowdown_err_x').value), 1.0)),
                        )
                        if abs(error_x) >= float(self.get_parameter('ball_arc_pivot_err_x').value):
                            heading_scale = 0.0

                    twist.linear.x = forward_sign * (base_forward * heading_scale)
                    if (
                        not close_area_mode
                        and not near_ball_mode
                        and 0.0 < abs(error_x) <= float(self.get_parameter('ball_far_turn_forward_max_err_x').value)
                        and 0.0 < abs(twist.linear.x) < float(self.get_parameter('ball_far_turn_forward_speed').value)
                    ):
                        twist.linear.x = forward_sign * float(self.get_parameter('ball_far_turn_forward_speed').value)
        ...
        if (
            self.state == self.APPROACH_BALL
            and (self.last_approach_was_straight or self.approach_near_ball_mode or self.approach_close_area_mode)
        ):
            min_linear = float(self.get_parameter('approach_centered_min_effective_linear_speed').value)
        ...
        if abs(scaled_angular_z) < 1e-6:
            ...
            if self.state == self.SEARCH_BALL:
                ...
                if self.state == self.APPROACH_BALL:
                    ...
                    if self.approach_near_ball_mode or self.approach_close_area_mode:
                        breakaway_speed = float(self.get_parameter('approach_close_turn_breakaway_speed').value)
                        hold_speed = float(self.get_parameter('approach_close_turn_hold_speed').value)
                    elif launch_active:
                    ...
                    if self.approach_turn_stuck_active:
                        if self.approach_near_ball_mode or self.approach_close_area_mode:
                            breakaway_speed = float(self.get_parameter('approach_close_turn_stuck_breakaway_speed').value)
                            hold_speed = float(self.get_parameter('approach_close_turn_stuck_hold_speed').value)
                        elif not launch_active:
